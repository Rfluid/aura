        ...
        let mut root = div()
            ...
            .on_children_prepainted(move |bounds, window, app| {
                ...
                if let Some(display) = app.primary_display() {
                    ...
                    let available_bottom = work_area::available_bottom(dbounds)
                        .map(px)
...
// ── Work-area detection (Strategy B) ──────────────────────────────────────────
//
// Inlined as a private submodule so `main.rs` and the cross-platform code
// path stay untouched. Returns the bottom Y of the available work area;
// the resize callback subtracts that from `window_top` to get the max
// growable height.
//
// Why config-file parsing instead of D-Bus: KDE Plasma 6's
// `org.kde.PlasmaShell.StrutManager.availableScreenRect` returns the full
// display rect on the versions tested (the strut manager is a setter
// registry that the read side doesn't actually subtract from the
// screen). The panels' thickness is, however, reliably persisted to
// `~/.config/plasmashellrc`, so we parse that file and correlate it with
...
mod work_area {
    ...
    ///
    /// `None` means "couldn't determine a panel reservation; fall back
    /// to whatever blind margin the caller has in mind". This happens
    /// on non-Linux platforms, on Linux without KDE Plasma config files
    /// present, or on any I/O / parse error.
    ///
    /// The lookup is cached process-wide after the first call. Users
    /// who resize their panel without restarting Aura get stale numbers
    /// — a fine tradeoff vs. re-reading the files on every resize frame.
    ...
    mod linux {
        ...
        /// thickest of several stacked panels). `None` means we tried
        /// and couldn't determine a reservation.
        ...
        /// IDs of `[Containments][N]` sections whose `location=4`
        /// (Plasma's bottom-anchor constant).
        ///
        /// Plasma's INI dialect lets sections nest like
        /// `[Containments][2][General]` — we only treat the exact
        /// two-part `[Containments][N]` headers as "panel root"; nested
        /// headers reset the current scope.
        ...
        /// `[PlasmaViews][Panel N][Defaults]` and per-screen overrides
        /// under `[PlasmaViews][Panel N][Screens][<name>]`. We take the
        /// max per ID so explicit overrides beat the default.
        ...
        mod tests {
            ...
            fn parses_bottom_containment_ids_only() {
                let path = std::env::temp_dir().join("aura-strategy-b-appletsrc");
                std::fs::write(
                    &path,
                    "\
[Containments][1]
...
",
            ...
            fn parses_panel_thicknesses() {
                let path = std::env::temp_dir().join("aura-strategy-b-plasmashellrc");
                std::fs::write(
                    &path,
                    "\
[PlasmaViews][Panel 2]
