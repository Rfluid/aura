...
// Store the MacPlatform pointer in a global instead of an NSApplication ivar.
// GPUI 0.2.2 originally subclassed NSApplication ("GPUIApplication") and stored
// the platform reference as an ivar on that subclass. On macOS 26 (Tahoe) the
// system calls +[NSApplication sharedApplication] before the Rust #[ctor] can
// register "GPUIApplication", so the subclass never takes effect and the ivar
// lookup panics. Removing the subclass entirely and using an AtomicPtr global
// fixes the panic without changing any observable behaviour.
static MAC_PLATFORM_PTR: AtomicPtr<c_void> = AtomicPtr::new(ptr::null_mut());
...
impl Platform for MacPlatform {
    ...
    fn run(&self, on_finish_launching: Box<dyn FnOnce()>) {
        ...
        unsafe {
            ...
            MAC_PLATFORM_PTR.store(self_ptr, Ordering::SeqCst);

            let pool = NSAutoreleasePool::new(nil);
            app.run();
            pool.drain();

            MAC_PLATFORM_PTR.store(null_mut(), Ordering::SeqCst);
        }
    }
...
fn get_mac_platform(_object: &mut Object) -> &MacPlatform {
    let platform_ptr = MAC_PLATFORM_PTR.load(Ordering::SeqCst);
    assert!(!platform_ptr.is_null(), "MacPlatform not initialised");
